...
/**
 * Recebe uma lista de id_unico e retorna apenas os que já estão cadastrados na base.
 */
export async function patientsExists(body: unknown): Promise<string[]> {
    ...
    for (let inicio = 0; inicio < idUnicos.length; inicio += TAMANHO_LOTE_FILTRO) {
        ...
        do {
            ...
            const resultadoQuery = await chamarNotionAPI(
                `databases/${tabelaPacientes.id}/query`,
                "POST",
                corpo
            );
    ...
    // Mantém a ordem de entrada e devolve só os cadastrados.
