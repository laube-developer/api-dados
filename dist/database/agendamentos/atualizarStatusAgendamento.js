...
export async function atualizarStatusAgendamento(id_unico, status) {
    if (!id_unico?.trim()) {
        throw new ErroValidacao("O campo 'id_unico' é obrigatório.");
    }
    if (!status || !STATUS_AGENDAMENTO_VALIDOS.includes(status)) {
        throw new ErroValidacao(`O campo 'status' é obrigatório e deve ser um dos seguintes: ${STATUS_AGENDAMENTO_VALIDOS.join(", ")}.`);
    }
...
//# sourceMappingURL=atualizarStatusAgendamento.js.map