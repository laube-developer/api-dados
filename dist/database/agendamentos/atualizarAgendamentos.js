...
function validarListaAgendamentos(agendamentos) {
    ...
    const agendamentosValidados = agendamentos.map((agendamento, indice) => {
        ...
        if (item.status !== undefined && !STATUS_AGENDAMENTO_VALIDOS.includes(item.status)) {
            erros.push(`O campo 'status' do item ${indice + 1} deve ser um dos seguintes: ${STATUS_AGENDAMENTO_VALIDOS.join(", ")}.`);
        }
...
async function buscarPaginaAgendamento(tabelaId, id_unico) {
...
export async function atualizarAgendamentos(agendamentos) {
...
//# sourceMappingURL=atualizarAgendamentos.js.map