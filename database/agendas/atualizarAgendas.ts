...
function validarListaAgendas(agendas: unknown): interfaces.AtualizacaoAgenda[] {
    ...
    const agendasValidadas = agendas.map((agenda, indice) => {
        const item = agenda as interfaces.AtualizacaoAgenda;
...
async function buscarPaginaAgenda(tabelaId: string, id_unico: string): Promise<any> {
...
export async function atualizarAgendas(agendas: unknown): Promise<interfaces.Agenda[]> {
    ...
    const agendasAtualizadas: interfaces.Agenda[] = [];
